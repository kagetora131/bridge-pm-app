...
- **タスクはメンバー起点で管理**: 各メンバーのカードに「担当タスク」として表示・追加。タイトル(日英)・説明(日英)・所属プロジェクト・開始日・期限・ステータス・依存タスク・翻訳ステータス・実績進捗を管理でき、依存タスクが未完了のまま残っている場合は警告表示。ステータスは一覧上のプルダウンで直接切り替えられる。担当者のいないタスクはメンバー画面下部の「未割当のタスク」欄に表示。
- **タスク詳細パネル(その場で編集)**: カレンダーのバー、ダッシュボードのタスク項目、メンバー画面のタスクをクリックすると右側に詳細パネルが開き、ステータス・担当者・日付・依存・翻訳をその場で編集できる(変更は自動保存。テキスト欄は入力欄を離れた時点で保存)。担当者の現地時刻・稼働状況、リスクの理由も表示。
- **実績進捗の入力と遅延検知**: 詳細パネルの0/25/50/75/100%ボタンで実績を記録。100%で「完了」、未着手に進捗を入れると「進行中」に自動で切り替わる。日付ベースの予定ペースと並べて表示し、実績が予定より20pt以上遅れていれば「遅れ気味」としてカレンダーの⚠やダッシュボードの「今日確認すべきこと」に含める(期限超過のタスクは「期限超過」のみ表示)。
- **安全な削除**: タスク・メンバー・プロジェクトの削除後、数秒間「元に戻す」が可能。関連タスクがあるメンバー/プロジェクトの削除時は、タスクを残して未割当/未所属にするか、一緒に削除するかを選べる。メンバー削除時はアサインと定例会議の参加者からも外す。
- **カレンダー(「進捗の羅列」ではなく「リスクの地図」)**: プロジェクト・メンバーで絞り込んだ上で、稼働中のタスクをプロジェクト色のコンパクトなバーで表示。**ブロック中・期限超過・依存タスク未解決・遅れ気味**のいずれかに該当するタスクだけを⚠付きの赤枠で強調し、リスクが埋もれないようにする。バー横の%は経過日数から算出した「予定ペース」の目安であり、人が入力する実績進捗(詳細パネルで入力)とは明確に分離して表示。メンバーが同時期に複数プロジェクトのタスクを抱えている「稼働の衝突」も検出して一覧表示。担当者の稼働曜日(`workingDays`)と祝日カレンダー(`src/lib/holidays.ts`)に基づき、週末・祝日は「休」の点線バーとして表示し、稼働しているように誤解されないようにする(拠点ごとの週末の違いにも対応。例: ドバイは金・土曜)。祝日は対応6拠点(日本・フランス・メキシコ・ニューヨーク/ロサンゼルス(米国)・ドバイ)の2026〜2028年分をタイムゾーンごとに保持。
- **メンバー管理**: 氏名・役割・拠点・タイムゾーン・稼働時間帯・稼働曜日・使用言語を登録し、各メンバーの現在時刻をリアルタイム表示。稼働曜日を持たせているのは、拠点によって週末が異なるケース(例: ドバイは金・土が休みで日曜が稼働日)に対応するため。
- **会議候補の複数案提示・公平性の可視化（差別化のコア機能）**: 参加メンバーと所要時間(30/45/60分)を選ぶと、「最適」「日本側早朝」「海外側時間外」の最大3案を提示(詳細は下記「会議提案ロジックの説明」)。各案の公平性スコア・参加者ごとの現地時間・時間外バッジ・今回の負担者を表示し、決定した案はログに記録して「過去の負担履歴」をメンバー別に可視化(特定拠点への偏りに気づける)。決定した候補はカレンダー登録用テキストとしてコピー可能。参加者は「プロジェクトのメンバー」「定例会議の参加者」単位でワンクリック選択でき、今後5日間それぞれの最適案を比較して公平性スコアが最も高い日を「おすすめ」表示する(開始時刻を過ぎた当日の枠は除外)。定例会議が参加者の祝日(各自の現地日付で判定)と重なる回は、週表示で黄色+⚠、一覧で今後8週間分の直近2件を警告する。
...
- **技術用語集**: 日英対訳＋メモを登録・検索。
- **多言語UI切り替え**: ヘッダーのボタンで日本語⇔英語をワンクリック切り替え。タスク名も表示言語の訳を優先して表示し、訳が無ければもう一方の言語で表示する。
- **自分視点(マイページ)**: ヘッダーの「視点」でメンバーを選ぶと、ダッシュボードがその人の現地時刻・稼働状況・今日の祝日/休日・週間稼働・未完了タスク(リスク順)・参加する定例会議(自分のタイムゾーン表示)をまとめたマイページに切り替わる。カレンダーの絞り込みと会議画面の表示タイムゾーンの初期値もその人に合わせる。
- **デモ体験の補助**: ヘッダーに「デモモード」バッジを常設し、localStorageのみで動作するデモであることを明示。3ステップの使い方ガイド(「使い方ガイド」ボタン)、全データのJSONエクスポート/インポート(バックアップ・移行用)を用意。

サンプルデータはゲーム開発スタジオを想定した架空のシナリオ([docs/bridge_pm_dummy_data.json](./docs/bridge_pm_dummy_data.json)、6拠点13名・過負荷2名・依存関係とブロック中タスクを含む)を元にしている。過負荷判定やドバイの稼働曜日は、このデータに付属する期待値と突き合わせて動作確認済み。タスクの英語タイトル・翻訳ステータス・実績進捗は、機能を試せるようデモ用に追加した値で、元のJSONには含まれない(ネオトーキョー2088は日本語へのフォールバック表示を確認できるよう未翻訳のまま)。
...
- 各メンバーの稼働時間帯は、稼働曜日(週末)に加えて祝日(`src/lib/holidays.ts`)も除外した上で算出するため、祝日には「候補時間帯なし」またはその人だけ時間外扱いになる。
...
- **カレンダーの%は「予定ペース」であり実績ではない**: 開始日〜期限の経過日数から機械的に算出した目安。実績進捗は人が詳細パネルで入力した値のみで、25%刻みの簡易入力。「遅れ気味」の判定(予定との差20pt以上)も単純な閾値による目安。
...
- **祝日データは6拠点(日本・フランス・メキシコ・米国・ドバイ)の2026〜2028年分のみ、静的に保持した参考値**: `src/lib/holidays.ts` にハードコードしており、外部の祝日APIには接続していない(完全無料の方針に合わせたオフライン動作)。日本の春分/秋分の日、ドバイのイスラム暦に基づく祝日(イード・アル=フィトル、イード・アル=アドハー等)は、公式発表(直前に確定)より前の推定値であり、`estimated: true` フラグを付与している。実運用では公式カレンダーで日付を確認・更新すること。対応拠点以外のタイムゾーンを持つメンバーを追加した場合、祝日は考慮されず稼働曜日のみが反映される。個別の有給休暇等は対象外。
